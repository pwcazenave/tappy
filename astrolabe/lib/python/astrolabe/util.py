...
from astrolabe.constants import pi2, minutes_per_day, seconds_per_day
...
def dms_to_d(deg, min, sec):
    ...
    """
    result = abs(deg) + abs(min) / 60.0 + abs(sec) / 3600.0
...
def load_params():
    ...
    """
    try:
        f = file(os.environ.get("ASTROLABE_PARAMS", "astrolabe_params.txt"))
    except IOError, value:
        print value
        raise Error, "Unable to open param file. Either set ASTROLABE_PARAMS correctly or create astrolabe_params.txt in the current directory"
    ...
    while token:
        if token == "standard_timezone_name":
            ...
            if direction == "east":
                longitude = -longitude
            astrolabe.globals.longitude = d_to_r(longitude)
        elif token == "latitude":
            ...
            if direction == "south":
                latitude = -latitude
            astrolabe.globals.latitude = d_to_r(latitude)
