...
dot = Literal(".")
piece = Word("KQBNR", max=1)
file_coord = Word("abcdefgh", max=1)
rank_coord = Word("12345678", max=1)
capture = Word("x:", max=1)
promote = Literal("=")
...
variant = Forward()
half_move = Combine((m1 | m2 | m3 | m4 | m5 | m6 | m7 | m8) + Optional(decoration)) \
