...
from math import *
import cPickle
from astrolabe.constants import pi2
from astrolabe.calendar import jd_to_jcent
from astrolabe.util import d_to_r, dms_to_d, polynomial, modpi2, diff_angle, ecl_to_equ
...
class VSOP87d:
    ...
    def __init__(self):
        ...
        We try to load a binary database. If that fails, we drop back to the
        text database.
        ...
        if not _first_time:
            return
        try:
            print "loading binary db..."
            load_vsop87d_binary_db()
            _first_time = False
        except:
            print "loading text db..."
...
def load_vsop87d_text_db():
    ...
    You would call it when building a binary version of the database; see:
    
        create_binary_vsop_db.py
        
    Or, when comparing the performance of loading the text and binary
    versions of the database; see:
    ...
    #
    # Prepare the dictionary with empty values
    #
    ...
    #     
    f = file(astrolabe.globals.vsop87d_text_path)
    lines = f.readlines()
    idx = 0;
    while idx < len(lines):
        planet, dim, it, nt = lines[idx].split()
        nt = int(nt)
        t = []
        for i in range(nt):
            idx += 1
            ...
            C = float(l[2])
            t.append((A, B, C))
        _planets[(planet, dim)].append(t)
        idx += 1
    f.close()


def write_vsop87d_binary_db():
    """Create a binary version of the VSOP87d database already in memory.
    
    You would call this routine only when building a binary version of the database; see:
    
        create_binary_vsop_db.py
        
    The database must already be in memory before calling this routine, presumably
    from loading the text version earlier. 
                
    """
    f = file(astrolabe.globals.vsop87d_binary_path, 'wb')
    cPickle.dump(_planets, f, True)
    f.close()


def load_vsop87d_binary_db():
    """Load the binary version of the VSOP87d database into memory.
    ...
    """
    global _planets
    f = file(astrolabe.globals.vsop87d_binary_path, 'rb')
    _planets = cPickle.load(f)
    f.close()
